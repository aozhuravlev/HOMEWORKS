...
class Model:
    ...
    def get_model_values(df_):
        ...
        def categorize(df_):
            df = df_.copy()
            categories = [i for i in df.columns if df[i].nunique() <= 4 and i != 'target']
            data_to_encode = df[categories]
            ohe = OneHotEncoder(sparse_output=False)
            ohe_encoded = ohe.fit_transform(data_to_encode)
            df_categorical = pd.DataFrame(ohe_encoded, columns=ohe.get_feature_names_out())
            df = pd.concat(
...
class knn_HW_functions:
    ...
    def get_my_df(from_text):
        ...
        def get_queries(txt):
            ...
            with open('data/sql_queries.txt', 'r') as file:
                queries = file.read().replace('\n', ' ').format(cols1, cols[-2], cols[-1]).split(';')
    ...
    def engineer_features(df_):
        ...
        def get_function_list():
            functions_list = [name for name, _ in inspect.getmembers(feat_eng_functions, inspect.isfunction)]
        ...
        for function_name in get_function_list():
            ...
            cols_to_drop = ['agreement_rk', 'dependants', 'loan_num_total', 'loan_num_closed', 'socstatus_work_fl', 'socstatus_pens_fl']

        return df.drop(columns=cols_to_drop)


    def multiple_scatter_plot(df, target):
        cols = [i for i in df.drop(columns=target).columns if df[i].dtype in ['float64', 'int64']]
        n_cols = 3
